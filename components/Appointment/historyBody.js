...
const HistoryBody = (props) => {
    ...
    console.log("IN THE HISTORY BODY")
    ...
    if (bookings.length>0 && appointments.length>0 && id!=null && datesFetched==false){
        ...
        for (var booking in clientBookings){
            ...
            for (var appointment in appointments){
                if (clientBookings[booking].AppointmentId == appointments[appointment].id){
                    ...
                    if (moment(apptDate, "YYYY-MM-DD").isBefore(currentTime, 'YYYY-MM-DD') && clientBookings[booking].Status=="Confirmed"){
                    ...
                    } else if (currentTime==apptDate && moment(appointments[appointment].StartTime, "h:mm A").isBefore(moment(apptStartTime, "h:mm A")) && clientBookings[booking].Status=="Confirmed"){
            ...
            {dateList.map((date) => {
                return(
                    <View style={styles.dateView}>
                        ...
                        {dates[date].map((row)=> {
                            return (
                                <BookingRow rowObj={row} cancelOption={false}/>
